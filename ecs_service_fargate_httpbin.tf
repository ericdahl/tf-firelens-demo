...
resource "aws_ecs_task_definition" "httpbin_fargate" {
  ...
  task_role_arn = "${aws_iam_role.task_httpbin_fargate.arn}"
}

resource "aws_iam_role" "task_httpbin_fargate" {
...
resource "aws_iam_policy" "task_httpbin_fargate" {
  name = "task_httpbin_fargate"
  policy = <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
...
resource "aws_iam_role_policy_attachment" "task_httpbin_fargate" {
  role = "${aws_iam_role.task_httpbin_fargate.name}"
  policy_arn = "${aws_iam_policy.task_httpbin_fargate.arn}"
}
...
resource "aws_cloudwatch_log_group" "httpbin_fargate_firelens" {
  count = "${var.enable_fargate_httpbin == "true" ? 1 : 0}"
...
resource "aws_cloudwatch_log_group" "httpbin_fargate_firelens_app" {
  count = "${var.enable_fargate_httpbin == "true" ? 1 : 0}"
...
output "aws_alb.httpbin_fargate.dns_name" {
  value = "http://${join(",", aws_alb.httpbin_fargate.*.dns_name)}"
}